...
# 3. 최종 에러 처리
ifndef COMPOSE
$(error "Neither podman nor docker found. Please install one of the container engines.")
endif
...
up: ## Run the application `make up cmd="crawl --channel CHANNEL financial-report --channel my_channel"`, open the Makefile to see more examples.
	@COMMAND="$(cmd)" $(COMPOSE) up --build app
...
shell: ## Run the shell
	@$(COMPOSE) build app
	@$(COMPOSE) run --name crawl-data-slack-shell --rm app bash

run: ## Run the cmd
	@$(COMPOSE) build app
	@$(COMPOSE) run --name crawl-data-slack-run --rm app $(cmd)
